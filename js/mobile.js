// ============================================================
// SOLESTE — Mobile Navigation (v2 — propre)
...
export function initMobileNav(activePage) {
  ...
  // Mettre à jour la bannière dès que le sélecteur est injecté
  const updateBanner = () => {
    const sel = document.getElementById('est-selector');
    const name = sel ? (sel.options[sel.selectedIndex]?.textContent || '') : '';
    if (name) { banner.textContent = name; drawerEst.textContent = name; }
  };
  const obs = new MutationObserver(updateBanner);
  obs.observe(document.querySelector('.header-right') || document.body, { childList: true, subtree: true });
  ...
  for (const link of navLinks) {
    if (link.section) navHtml += `<div class="nav-section">${link.section}</div>`;
  ...
  drawer.innerHTML = `
    ...
    <div class="mob-drawer-est" id="mob-drawer-est">…</div>
    <nav>${navHtml}</nav>`;
  ...
  let tx = 0;
  drawer.addEventListener('touchstart', e => { tx = e.touches[0].clientX; }, { passive: true });
  drawer.addEventListener('touchend',   e => { if (tx - e.changedTouches[0].clientX > 60) closeDrawer(); }, { passive: true });
}
